...
void Chassis::drive(int8_t axial, int8_t lateral, int8_t yaw) {
  long frontleftPower = axial - lateral - yaw;
  long frontrightPower = axial + lateral + yaw;
  long backleftPower = axial + lateral - yaw;
  long backrightPower = axial - lateral + yaw;
  ...
  if (maxPower > MAX_POWER) {
    ...
    frontrightPower =
        map(frontrightPower, -maxPower, maxPower, -MAX_POWER - 1, MAX_POWER);
    backleftPower =
